...
Refactoring code can make computing more efficient and reduce the load on the computer's processor, however, it can also be timely and costly to a company or client.

For the client in this scenario, the refactored code can analyze large amounts of stock data while comprising less computer processing memory to do so. This is a big advantage if there are vast amounts of data for the program to run through. The original data for 2017 analyzed 12 types of stock which was about 3,000 rows. This took about about .83 seconds. Imagine if this same program were to analyze 3,000 stock types, at the rate of 12 tickers per 0.83 seconds, this would take about 3 1/2 minutes for the program to run through all the types of stock. It could also take up a lot of processing memory. If we were to run the same 3,000 stock types at a rate of 12 tickers per 0.14 seconds, we could run all 3,000 types of stock in about 35 seconds. That is a significant difference.
